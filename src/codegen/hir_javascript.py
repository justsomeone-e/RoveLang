"""ECMAScript 2022 / Node.js emission from verified Rove HIR."""
...
_JAVASCRIPT_RUNTIME = r'''// Auto-generated by Rove Compiler (js HIR backend)
// Target: ECMAScript 2022 / Node.js
"use strict";

const _roveFs = require("node:fs");
const _roveChildProcess = require("node:child_process");
...
function _rovePropagate(result) {
...
function _roveCloneValue(value, seen = new Map()) {
    ...
    for (const key of Object.keys(value)) clone[key] = _roveCloneValue(value[key], seen);
    return clone;
}

function _roveStringLength(value) { return _roveI64(BigInt(Array.from(value).length)); }
function _roveStringIndex(value, index) { return Array.from(value)[Number(index)] ?? ""; }
...
function _roveI64Mul(left, right) { return _roveI64(left * right); }
function _roveI64Neg(value) { return _roveI64(-value); }
function _roveI64Not(value) { return _roveI64(~value); }
function _roveI64And(left, right) { return _roveI64(left & right); }
function _roveI64Xor(left, right) { return _roveI64(left ^ right); }
function _roveI64Or(left, right) { return _roveI64(left | right); }
function _roveI64Shl(value, amount) { return _roveI64(value << (amount & 63n)); }
function _roveI64Shr(value, amount) { return _roveI64(_roveI64(value) >> (amount & 63n)); }
function _roveIntDiv(left, right) {
    if (right === 0n) throw new RangeError("integer division by zero");
    return _roveI64(left / right);
}
function _roveIntMod(left, right) {
    if (right === 0n) throw new RangeError("integer division by zero");
    return _roveI64(left % right);
}
function _roveF64Div(left, right) { return Number(left) / Number(right); }
function _roveF64Mod(left, right) { return Number(left) % Number(right); }
function _roveNormalizeDecimal(text) {
...
function to_int(value) {
    ...
    try { return _roveI64(BigInt(text)); } catch { return 0n; }
}
...
function _rove_bootstrap_read_file(path) {
    try { return _roveFs.readFileSync(path, "utf8"); } catch { return ""; }
}
function _rove_bootstrap_write_file(path, content) {
    try { _roveFs.writeFileSync(path, content, "utf8"); return true; } catch { return false; }
}
function _rove_bootstrap_file_exists(path) { return _roveFs.existsSync(path); }
function _rove_bootstrap_remove_file(path) {
    try { _roveFs.unlinkSync(path); return true; } catch { return false; }
}
...
function _rove_toolchain_compile_cpp(source, output) {
    ...
    for (const compiler of candidates) {
        const result = _roveChildProcess.spawnSync(
...
function len(value) {
    ...
    return typeof value === "string" ? _roveStringLength(value) : _roveI64(BigInt(value.length));
}
function args() { return process.argv.slice(); }
function input(prompt = "") {
    if (prompt) process.stdout.write(String(prompt));
    if (input._lines === undefined) {
        input._lines = _roveFs.readFileSync(0, "utf8").split(/\r?\n/);
    }
    return input._lines.shift() ?? "";
}

let _roveAddress = 0n;
function addr(_value) { _roveAddress = _roveI64Add(_roveAddress, 1n); return _roveAddress; }
...
function memdump(address, _length = 16) {
    ...
    print(`0x${rendered}: 52 6F 76 65                                      | Rove`);
}
...
class _RoveChannel {
    ...
    receive() {
        if (this.values.length === 0) throw new Error("receive from empty Rove channel");
        return this.values.shift();
    }
}
function channel(..._typeArguments) { return new _RoveChannel(); }

function _roveRunDefers(stack) {
...
const _rove_time_now_ms = () => _roveI64(BigInt(Date.now()));
const _rove_time_now_us = () => _roveI64(BigInt(Math.trunc(performance.now() * 1000)));
...
const _rove_process_exec_cmd_result = command => {
    try {
        _roveChildProcess.execSync(command, { stdio: "pipe" });
        return Ok(0n);
    } catch (error) {
...
const _rove_json_get_int_result = (document, key) => {
    ...
    try { const value = BigInt(match[0]); if (value < -(1n << 63n) || value > (1n << 63n) - 1n) return Err(`JSON integer is out of range: ${key}`); return Ok(_roveI64(value)); }
...
class HIRJavaScriptEmitter:
    ...
    def emit(self) -> str:
        ...
        if self.esm:
            lines.extend((
                "import { createRequire as _roveCreateRequire } from 'node:module';",
                "const require = _roveCreateRequire(import.meta.url);",
                "",
            ))
    ...
    def _emit_declaration(self, node: IRNode) -> List[str]:
        ...
        if isinstance(node, IREnum):
            if any(member.is_variant for member in node.members):
                ...
                for member in node.members:
                    ...
                    params = [f"_rovePayload{index}" for index in range(len(member.payload_types))]
        ...
        if isinstance(node, IRExternFunction):
            ...
            if node.varargs:
                parameters.append("..._roveVarargs")
            return [
    ...
    def _emit_block(
        ...
        *,
        ...
        if any(isinstance(statement, IRDefer) for statement in statements):
            ...
            lines.append(f"{prefix}    _roveRunDefers({stack});")
            lines.append(f"{prefix}}}")
            return lines
        lines: List[str] = []
        for statement in statements:
    ...
    def _expr(self, node: IRExpr | None) -> str:
        ...
        if isinstance(node, IRBinary):
            ...
            if node.type.name == "float" and node.op == "/":
                return f"_roveF64Div({left}, {right})"
            if node.type.name == "float" and node.op == "%":
                return f"_roveF64Mod({left}, {right})"
            operator = {
                "and": "&&",
                "or": "||",
                "==": "===",
        ...
        if isinstance(node, IRUnary):
            ...
            if node.type.name == "int":
                if node.op == "-":
                    return f"_roveI64Neg({self._expr(node.expr)})"
                if node.op == "~":
                    return f"_roveI64Not({self._expr(node.expr)})"
                if node.op == "+":
        ...
        if isinstance(node, IRResultPropagate):
            return f"_rovePropagate({self._expr(node.expr)})"
        ...
        if isinstance(node, IRIndexAccess):
            if node.obj.type.name == "string":
                return f"_roveStringIndex({self._expr(node.obj)}, {self._expr(node.index)})"
    ...
    def _copy_value(self, rendered: str, value_type: IRType | None) -> str:
        ...
        if value_type.name == "Array" or value_type.name in self.structs:
            return f"_roveCloneValue({rendered})"
