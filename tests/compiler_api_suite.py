...
from src import CompilerPlugin, RoveCompiler, check_source, compile_source
from src.core.diagnostics import DiagnosticEmitter


def run_compiler_api_suite() -> bool:
    print("=" * 70)
    print("ROVE PUBLIC COMPILER API / EMBEDDING CONTRACT")
    print("=" * 70)
    ...
    expected = {
        ...
        "python": (".py", "def add"),
        "rust": (".rs", "pub fn add"),
        "react": (".tsx", "export default function RoveApp"),
        "wasm": (".wat", "(module"),
    ...
    for target, (extension, marker) in expected.items():
        ...
        if target == "wasm":
            assert "Auto-generated by rove bundle (rove_module)" in result.artifact.content
    ...
    with contextlib.redirect_stdout(captured), contextlib.redirect_stderr(captured):
        invalid = check_source(invalid_source, filename="invalid.rove")
    ...
    assert diagnostic.path == "invalid.rove"
    ...
    def reject_once(_: int) -> str:
        result = compiler.check_source(invalid_source, filename="parallel_invalid.rove")
